...
const FIREDANCER_PACKET_SZ: u64 = 32 + FIREDANCER_STAKE_WEIGHT_CNT * 40;
...
impl LeaderScheduleCache {
    ...
    pub fn new(epoch_schedule: EpochSchedule, root_bank: &Bank, firedancer_send: bool) -> Self {
        ...
        unsafe { Self::firedancer_send_leader_schedule( leader_schedule_epoch-1, root_bank, &mut cache.firedancer_rwlock ); }
        cache
    }
    ...
    fn compute_epoch_schedule(&self, epoch: Epoch, bank: &Bank, rooted: bool) -> Option<Arc<LeaderSchedule>> {
        ...
        leader_schedule.map(|leader_schedule| {
            ...
            if rooted {
                unsafe{ Self::firedancer_send_leader_schedule( epoch, bank, &self.firedancer_rwlock ); }
            }
            leader_schedule
        })
    }
    ...
    unsafe fn firedancer_send_leader_schedule(
        ...
        if let Some(lock) = send_firedancer.as_ref() {
            ...
            let (first_slot, slot_cnt) = (bank.epoch_schedule().get_first_slot_in_epoch(epoch), bank.epoch_schedule().get_slots_in_epoch(epoch));
            let stakes = bank.epoch_staked_nodes(epoch).map(|stakes| {
            ...
            let mut memory: [u8; FIREDANCER_PACKET_SZ as usize] = [0; FIREDANCER_PACKET_SZ as usize];

            if stakes.len() > FIREDANCER_STAKE_WEIGHT_CNT as usize {
                panic!("unable to communicate leader schedule to Firedancer, too many stake weights");
            }
            ...
            fd_ext_poh_publish_leader_schedule(memory.as_ptr(), 32 + stakes.len() as u64 * 40);
        }
    }
...
mod tests {
    ...
    fn run_thread_race() {
        ...
        let cache = Arc::new(LeaderScheduleCache::new(epoch_schedule, &bank, true));
    ...
    fn test_next_leader_slot_next_epoch() {
        ...
        let schedule = cache.compute_epoch_schedule(epoch, &bank, true).unwrap();
        let mut index = 0;
